...
export class AuthorizationFactory {
    ...
    private createPresenter = () => {
        ...
        const authorizeUseCase = new AuthorizeUseCase(authorizeRequester, userPersistence);

        const state = new AuthorizationState(errorStore, isLoadingStore, isDisabledStore, emailStore, passwordStore);
        const controller = new AuthorizationController(state, authorizeUseCase, config.REGEX);
